...
floats = st.floats(
    allow_infinity=False, allow_nan=False, min_value=-1e15, max_value=1e15
)
...
@given(value=integers, name=simple_strings)
...
@settings(max_examples=100)
@given(value=floats, name=simple_strings)
...
@settings(max_examples=100)
@given(value1=simple_strings, value2=integers)
...
@given(name=simple_strings, value=integers)
...
def test_float_parsing_all_values(value):
    ...
    if result is not None:
        ...
        assert (
            abs(result.named["value"] - value) < 1e-10 or result.named["value"] == value
        )


@settings(max_examples=100)
@given(value=floats, precision=st.integers(min_value=1, max_value=10))
...
@given(
    value=st.one_of(
        st.floats(
            min_value=1e-10, max_value=1e10, allow_infinity=False, allow_nan=False
        ),
        st.floats(
            min_value=-1e10, max_value=-1e-10, allow_infinity=False, allow_nan=False
        ),
    )
)
def test_scientific_notation_parsing(value):
    ...
    if result is not None:
        ...
        assert (
            abs(result.named["value"] - value) < abs(value) * 1e-6
            or result.named["value"] == value
        )
...
@settings(max_examples=100)
@given(
    fill_char=st.sampled_from(
        [".", "x", "-", "_"]
    ),  # Exclude '0' to avoid confusion with numbers
    alignment=st.sampled_from(["<", ">", "^"]),
    width=st.integers(min_value=5, max_value=15),
    value=simple_strings.filter(
        lambda s: len(s) > 0 and len(s) < 10 and s.strip()
    ),  # Ensure value fits and is not whitespace-only
)
def test_fill_char_combinations(fill_char, alignment, width, value):
    ...
    assume(
        value.strip()
    )  # Skip whitespace-only strings (they may be stripped during parsing)
...
@settings(max_examples=100)
@given(
    prefix=simple_strings.filter(
        lambda s: not any(c.isdigit() for c in s)
    ),  # Avoid digits in prefix
    value=integers,
    suffix=simple_strings.filter(
        lambda s: not any(c.isdigit() for c in s)
    ),  # Avoid digits in suffix
)
...
def test_findall_with_separators(separator, values):
    ...
    for val in values:
        assert val in found_values, (
            f"Value {val} not found in {found_values} for text {text!r}"
        )
...
@given(
    text_parts=st.lists(
        simple_strings.filter(lambda s: not any(c.isdigit() for c in s)),
        min_size=2,
        max_size=5,
    ),
    value=integers,
...
@settings(max_examples=100)
@given(
    name=simple_strings.filter(
        lambda s: s.isalpha() and s.lower() != s.upper()
    ),  # Only letters with case differences
    value=integers,
)
def test_case_sensitive_matching(name, value):
    """Property: Case-sensitive matching should respect exact case"""
    pattern = "Name: {name}, Value: {value:d}"
...
@settings(max_examples=100)
@given(name=simple_strings.filter(lambda s: s.isalpha()), value=integers)
def test_case_insensitive_matching(name, value):
    ...
    for case_func in [str.upper, str.lower, str.capitalize]:
        ...
        if result is not None:
            ...
            assert (
                result.named["name"].lower() == name.lower()
                or result.named["name"] == name
            )
...
@settings(max_examples=50)
@given(values=st.lists(st.integers(min_value=0, max_value=100), min_size=1, max_size=5))
