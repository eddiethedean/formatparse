...
@given(
    pattern=st.text(min_size=1, max_size=200), text=st.text(min_size=0, max_size=1000)
)
...
@given(pattern=st.text(min_size=1, max_size=200))
...
@given(
    ...
    ),  # Remove filter to reduce filtering overhead
...
@given(
    ...
    )  # Moderate large inputs (reduced to avoid memory issues)
)
...
@given(
    text=st.text(
        min_size=1,
        max_size=50,
        alphabet=st.characters(min_codepoint=32, max_codepoint=0x7F),
    )  # Limit to ASCII for simpler testing
)
...
@given(
    text=st.text(
        ...
        max_size=100,
        alphabet=st.characters(min_codepoint=32, max_codepoint=0x7F),
    ),  # Limit to ASCII to avoid byte boundary issues
