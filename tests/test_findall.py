...
def test_findall():
    s = "".join(r.fixed[0] for r in findall(">{}<", "<p>some <b>bold</b> text</p>"))
...
def test_no_evaluate_result():
    s = "".join(
        ...
        for m in findall(">{}<", "<p>some <b>bold</b> text</p>", evaluate_result=False)
    )
    assert s == "some bold text"
...
def test_case_sensitivity():
    ...
    results = [r.fixed[0] for r in findall("x({})x", "X(hi)X", case_sensitive=True)]
    assert results == []
