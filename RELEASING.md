...
The canonical version is **`0.8.6`** in the workspace [`Cargo.toml`](Cargo.toml) (`[workspace.package] version` and `[workspace.dependencies] formatparse-core` must match). Member crates use `version.workspace = true`. PyPI metadata uses `dynamic = ["version"]`; Maturin reads the workspace version when building wheels and sdist.

**PyPI / crates.io latest published:** `0.8.5` (tag `v0.8.5`). For the next release, bump the workspace version in `Cargo.toml` and `CHANGELOG.md`, then tag `vX.Y.Z` (see [Tag without a version bump](#tag-without-a-version-bump)).

Before tagging, confirm:

- `[workspace.package] version` and `formatparse-core` dependency version in root [`Cargo.toml`](Cargo.toml) are both **`0.8.6`**.
- [`CHANGELOG.md`](CHANGELOG.md) lists all 0.8.6 changes under `## [0.8.6]` (not only under `[Unreleased]`).
- CI on `main` is green.
