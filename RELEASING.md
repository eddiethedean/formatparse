...
The canonical version is **`0.8.7`** in the workspace [`Cargo.toml`](Cargo.toml) (`[workspace.package] version` and `[workspace.dependencies] formatparse-core` must match). Member crates use `version.workspace = true`. PyPI metadata uses `dynamic = ["version"]`; Maturin reads the workspace version when building wheels and sdist.

**PyPI latest published:** `0.8.6` (tag `v0.8.6`). Workspace is already at **`0.8.7`**; after CI is green on `main`, tag `v0.8.7` to publish (see [Tag without a version bump](#tag-without-a-version-bump)).

Before tagging, confirm:

- `[workspace.package] version` and `formatparse-core` dependency version in root [`Cargo.toml`](Cargo.toml) are both **`0.8.7`**.
- [`CHANGELOG.md`](CHANGELOG.md) lists all 0.8.7 changes under `## [0.8.7]` (not only under `[Unreleased]`).
- CI on `main` is green.
