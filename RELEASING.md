...
The canonical version is **`0.8.3`** in the workspace [`Cargo.toml`](Cargo.toml) (`[workspace.package] version` and `[workspace.dependencies] formatparse-core` must match). Member crates use `version.workspace = true`. PyPI metadata uses `dynamic = ["version"]`; Maturin reads the workspace version when building wheels and sdist.

**PyPI / crates.io latest published:** `0.8.2` (tag `v0.8.2`). To ship 0.8.3, tag `v0.8.3` after the checklist below—do **not** run `release.sh 0.8.3` (that would create a redundant version commit); use [Tag without a version bump](#tag-without-a-version-bump) instead.

Before tagging, confirm:

- `[workspace.package] version` and `formatparse-core` dependency version in root [`Cargo.toml`](Cargo.toml) are both **`0.8.3`**.
- [`CHANGELOG.md`](CHANGELOG.md) lists all 0.8.3 changes under `## [0.8.3]` (not only under `[Unreleased]`).
- CI on `main` is green.
